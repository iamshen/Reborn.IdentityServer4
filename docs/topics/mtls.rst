...
In IdentityServer, the mutual TLS endpoints, can be configured in three ways (assuming IdentityServer is running on ``https://IdentityServer4.io``:

* path-based - endpoints located beneath the path ``~/connect/mtls``, e.g. ``https://IdentityServer4.io/connect/mtls/token``.
* sub-domain based - endpoints are on a sub-domain of the main server, e.g. ``https://mtls.IdentityServer4.io/connect/token``.
...
Below is an example how an API in ASP.NET Core might be configured for both access tokens and client certificates::
    ...
    services.AddAuthentication("token")
        ...
        {
            options.Authority = "https://IdentityServer4.io";
