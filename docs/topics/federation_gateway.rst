...
Implementation
^^^^^^^^^^^^^^
Our `quick start UI <https://github.com/iamshen/IdentityServer8.Quickstart.UI>`_ utilizes some of the below features. Also check out the :ref:`external authentication quickstart <refExternalAuthenticationQuickstart>` and the 
